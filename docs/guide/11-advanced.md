...
| 9 | 问答双模式:自研 pipeline 与 LangChain 框架并存,同库同 Prompt | 把「框架帮你做了什么」变成可体验的对照实验;代价是多一条生成链路,且 LangChain 沦为可选依赖需处理未安装场景(import 内聚在构造函数 + 500 带安装指引) |
...
整个项目约 800 行 Python + 一个 React 前端,自研主链路没有一行 RAG 逻辑来自框架 —— 同时附上 LangChain 框架模式作为「对照组」,让这个结论可以被亲手验证。回头看会发现:RAG 的「神秘感」其实由四件事构成 —— **好的分块、准确的向量检索、约束严格的 Prompt、清晰的编排**。这四件事,你都已经亲手写过一遍了;再切换到框架模式看看,它替你包办的也正是这几件。
